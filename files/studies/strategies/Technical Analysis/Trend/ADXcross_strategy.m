function [params,sig] = ADXcross_strategy(x,price)
%% ADX - Wildmer's Averaged Directional Index strategy
...
% OPTIMIZATION RANGE
params.per = [2:2:12, 15:2:35, 40:5:150];                     % period of ADX
...
output = indicators([price.High,price.Low,price.Close],'adx',per);
pdi = output(:,1); % +DI
mdi = output(:,2); % -DI
...
% RULES
sig(pdi >= mdi) = 1; % buy 1 amount of asset
sig(pdi < mdi) = -1; % sell short 1 amount of asset
