function [params,sig] = OptimalMAcross_strategy(x,price)
%% Optimal Moving Averages crossover strategy
...
% OPTIMIZATION RANGE
params.ma_num = 1:12;                                       % Moving Average type
...
ma_type = {'dema','tema','ehma','whma','trix' ...
    'modified','linear','simple','exponential','square','square-root','triangular'};
if ma_num < 6
    lead_ma = I_Complex_MA(ma_type{ma_num},price.Price,lead);
    lag_ma = I_Complex_MA(ma_type{ma_num},price.Price,lag);
else
    lead_ma = movavg(price.Price,ma_type{ma_num},lead);
    lag_ma = movavg(price.Price,ma_type{ma_num},lag);
end
