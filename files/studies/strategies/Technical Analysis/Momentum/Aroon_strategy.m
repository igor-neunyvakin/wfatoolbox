function [params,sig] = Aroon_strategy(x,price)
...
output = indicators([price.High,price.Low,price.Close],'aroon',per);
dn = output(:,1);
up = output(:,2);
os = output(:,3);
...
% RULES
sig(up >= 80) = 1; % buy 1 amount of asset
sig(dn < 20) = -1; % sell short 1 amount of asset
