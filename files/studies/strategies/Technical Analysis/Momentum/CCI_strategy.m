function [params,sig] = CCI_strategy(x,price)
...
% OPTIMIZATION RANGE
params.tp_per = [2:2:12, 15:5:35, 40:10:50];        % period of lead moving average
params.md_per =  [5:2:12, 15:5:50, 60:10:100];       % period of lag moving average
params.const =  0.005:0.005:0.05;
...
cci = indicators([price.High,price.Low,price.Close],'cci',tp_per,md_per,const);
...
for i = 2:length(price.Close)
    if cci(i) > upper_thresh && cci(i-1) < upper_thresh
        sig(i) = -1;
    elseif cci(i) < lower_thresh && cci(i-1) > lower_thresh
        sig(i) = 1;
    else
        sig(i) = sig(i-1);
    end
