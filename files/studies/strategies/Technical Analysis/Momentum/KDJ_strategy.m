function [params,sig] = KDJ_strategy(x,price)
%% %K %D %J Oscillator strategy
...
output = indicators([price.High,price.Low,price.Close],'kdj',k,d);
jline = output(:,3);
...
for i = 2:length(price.Close)
    if jline(i) > upper_thresh && jline(i-1) < upper_thresh
        sig(i) = -1;
    elseif jline(i) < lower_thresh && jline(i-1) > lower_thresh
        sig(i) = 1;
    else
        sig(i) = sig(i-1);
    end
