function [params,sig] = KeltnerChannels_strategy(x,price)
...
% OPTIMIZATION RANGE
params.emaper = [2:2:12, 15:3:35, 40:5:60];        % number of periods for ema
params.atrmul =  [2:2:12, 15:3:35, 40:5:60];         % atr multiplier
params.atrper =  [2:2:12, 15:3:35, 40:5:60];         % number of periods for atr
...
output = indicators([price.High,price.Low,price.Close],'keltner',emaper,atrmul,atrper);
middle = output(:,1);
upper = output(:,2);
lower = output(:,3);
...
for i = 2:length(price.Price)
    % sell
    if price.Price(i) > upper(i) && price.Price(i-1) < upper(i-1) % when crossover upper line
        sig(i) = -1;
    % buy
    elseif price.Price(i) < lower(i) && price.Price(i-1) > lower(i) % when crossover lower line
        sig(i) = 1;
    % close
    elseif (sig(i-1) < 0 && price.Price(i) < middle(i)) || ...
              (sig(i-1) > 0 && price.Price(i) > middle(i))         % when crossover middle line
        sig(i) = 0;
    else
        sig(i) = sig(i-1);
    end
