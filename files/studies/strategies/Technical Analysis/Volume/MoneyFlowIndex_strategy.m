function [params,sig] = MoneyFlowIndex_strategy(x,price)
...
mfi = indicators([price.High,price.Low,price.Close,price.Volume],'mfi',period);
...
for i = 2:length(price.Close)
    % sell
    if mfi(i) < upper_thresh && mfi(i-1) > upper_thresh
        sig(i) = -1;
    % buy
    elseif mfi(i) > lower_thresh && mfi(i-1) < lower_thresh
        sig(i) = 1;
    % close
    elseif (sig(i-1) < 0 && mfi(i) > 50) || ...
              (sig(i-1) > 0 && mfi(i) < 50)
        sig(i) = 0;
    else
        sig(i) = sig(i-1);
