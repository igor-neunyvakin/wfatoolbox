function [params,sig] = ForceIndex_strategy(x,price)
...
% CHECK PRICE
global start_btn
if ~isfield(price,'Volume') || ~isfield(price,'Close')
    [~,strat_name] = fileparts(mfilename('fullpath'));
    msgbox(['To run the strategy',strat_name,' you need to load Volume and Close data'])
    start_btn = 0;
    return
end
...
force = indicators([price.Close,price.Volume],'force',period);
...
% RULES
sig(force >= 0) = 1; % buy 1 amount of asset
sig(force < 0) = -1; % sell short 1 amount of asset
