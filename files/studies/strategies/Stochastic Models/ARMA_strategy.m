function [params,sig] = ARMA_strategy(x,price)
...
% OPTIMIZATION RANGE
params.p = 2;     % no params
params.q = 5;     % no params
...
% MODEL
model=arima(p, 0, q);   % ARMA
fit = estimate(model, price.Price,'Display','off');

% forecast
yF = NaN(size(price.Price));
for t = p:size(price.Price, 1)
    [y, ~] = forecast(fit, 1, 'Y0', price.Price(t-p+1:t));
    yF(t)=y(end);
end
...
% RULES
deltaYF = yF-price.Price;
sig(deltaYF > 0) = 1;      % buy 1 amount of asset
sig(deltaYF < 0) = -1;    % sell short 1 amount of asset
